...
% change the length of the asperity:
% ----------------------------------
% absoluteAsperityLenghtArray   = absoluteAsperityLength + 0.001*(1:numberOfRuns);
...
% asperityTypeArray             = {'sine','triangle', 'box','step','jog'}; 
% if length(asperityType) ~= numberOfRuns 
%     error('asperityTypes must be of length number of runs')
% end
...
% run the work flow
tic
growOutput = fric2d_workflow(fileName,X,Y);
runTime = toc;
