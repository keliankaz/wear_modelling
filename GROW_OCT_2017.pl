...
sub grow
{
    ...
    my $raw_output = "Executing GROW version Oct. 2017\n\n"
...
sub tune_angles
{
	...
	foreach (@angles)
	{
		...
		# normalize work by area of newly added elements= get delwext/A
	...
	{
	    %this_fault = remove_not_slipping_ineff($is_min, \%this_file, \%this_fault)
	    #%this_fault = remove_not_slipping_ineff(\%this_file, \%this_fault);
	}
	...
	{	    
	    ...
	    # do not forecast within tuning step
	    if ($forecast)
	    {
		#($eff_angle, $eff_work) = get_PDF_forecast(\%valid_works, $prev_prop_work_unnorm, $forecast, $is_min);
	    }
	    else
	    {
		# if we apply displacement boundary conditions, 
	...
	if ($eff_angle > 0 && ($is_min && $eff_work < $final_work || (!$is_min && $eff_work > $final_work)))
	{
	    ...
	    my $out = "\nTUNING PROPAGATION: $prop_num \nMORE EFFICIENT delWext/delA CALCULATED WHILE TUNING: $final_work (J/m^2)\nRESETTING MOST EFFICENT GEOMETRY TO FILE: $seq_filename \n\tEfficient Geometry \n";
	    $out = $out.sprintf("\t%-10s %-10s\n", "Fault", "Angle");
...
sub normalize_work
{
    ...
    #print "prev work: $prev_work \n";
    
    #print "current file: $testing_filename \n";
...
# input: reference to hash table containing scenario index numbers
#        => work at this iteration, whether work should be min or maxed
# output:  index,  work calced for PDF determined path
sub get_PDF_forecast
{
    my ($hash_ref, $prev_work, $forecast, $is_min) = @_;
    my %hash = %$hash_ref; 
       
    # make this an input at some point
    my @fore_data = split(/\s+/, $forecast);
    my $PDF_percent = $fore_data[-1];
    ...
    my @delworks = ();
    my $i = 0;
    # make a list of angles and work
    foreach my $ang (sort {$a <=> $b} keys %hash)
    {
	my $work= $hash{$ang};
	$angs[$i] = $ang;
	$delworks[$i] = abs($prev_work - $work);
	$i++;
    }

    # make fake histogram of data
    my @weights = ();
    my @angs_weights = ();
    my $max_delwext = max_list(@delworks);
    $i = 0;
    while ($i <= $#delworks)
    {
	my $ang_curr = $angs[$i];
	my $delwext_curr = $delworks[$i];
	my $num_ang = ceil(($delworks[$i]/$max_delwext)*100);
	my $wi = 0;
	while ($wi <= $num_ang)
	{
	    push(@angs_weights, $ang_curr);
    ...
    # calc PDF from fake data
    # bandwidth
    my $h = 55; ## turns out very different function when tuning angles not included 
    # number of angles in fake data
    my $n = $#angs_weights+1;
    my $coeff = 1/($h*$n);
    
    # angles for which to calc prob.
    my @angs_PDF = ();
    my @PDF = ();
    my $a_min = min_list(keys %hash);
    my $a_max = max_list(keys %hash);
    my $a_inc = 1;

    my $a_curr = $a_min;
    while ($a_curr <= $a_max)
    {
	push(@angs_PDF, $a_curr);
	$a_curr = $a_curr + $a_inc;
    }
	...
	my $i_fake = 0;
	while ($i_fake <= $#angs_weights)
	{
	    my $ang_j = $angs_weights[$i_fake];
	    my $u = ($ang_x-$ang_j)/$h;
	    my $u_ind = 0;
	    if (abs($u) <= 1)
	    {
	    ...
	    my $K_curr = 0.75*(1-($u**2))*$u_ind;

	    $K_sum = $K_sum + $K_curr;
	    $i_fake++;
	}	

        my $kern_x = $K_sum*$coeff;
	push(@PDF, $kern_x);
	$i++;
    }


    my $PDF_max = max_list(@PDF);
    my $PDF_env = $PDF_max*$PDF_percent;
    my @angs_env = ();
    my @PDF_env = ();
    $i = 0;
    print "\n\nPDF------------------\n";
    # find angles in PDF equal or above within or above PDF envelope value
    while ($i <= $#angs_PDF)
    {	
	print "PDF    $angs_PDF[$i] $PDF[$i]\n";
	my $curr_PDF = $PDF[$i];
	my $curr_ang = $angs_PDF[$i] ;
	if ($curr_PDF >= $PDF_env)
	{
	    push(@angs_env, $curr_ang);
	    push(@PDF_env, $curr_PDF);
	}
	$i++;
    }

    print "PDF angles in PDF envelope: @angs_env \n";

    my $ang_PDF_env_min = min_list(@angs_env);
    my $ang_PDF_env_max = max_list(@angs_env);

    # select largest or smallest angle
    # w/r/t to fault tip with 
    my $is_min_ang = 1;
    if ($forecast =~ /max/)
    {
	$is_min_ang = 0;
    ...
    my @PDF_env_angs = ();
    # for all the angles in the real data set (the hash table)
    # find angles that are closest to min and max PDF values
    my $min_dist = 360;
    my $near_ang = -1;
    foreach my $ang_h (sort {$a <=> $b} keys %hash)
    {
	# find angle in PDf closest to hash angle
	if ($is_min_ang && (abs($ang_PDF_env_min - $ang_h) < $min_dist))
	{
	    $near_ang = $ang_h;
	    $min_dist = abs($ang_PDF_env_min - $ang_h);
	}
	elsif (!$is_min_ang && (abs($ang_PDF_env_max - $ang_h) < $min_dist))
	{
	    $near_ang = $ang_h;
	    $min_dist = abs($ang_PDF_env_max - $ang_h);
	}
	    $i++;
    }

    if ($is_min_ang == 1 && $near_ang > -1)
    {
	print "PDF closest min angle to PDF envelope= $near_ang \n";
	print "PDF------------------\n\n";
	return ($near_ang, $hash{$near_ang});
    }
    elsif ($is_min_ang == 0 && $near_ang > -1)
    {
	print "PDF  closest max angle to PDF envelope= $near_ang \n";
	print "PDF------------------\n\n";
	return ($near_ang, $hash{$near_ang});
	# could interpolate to get value of wext in order to sample angles in pdf between data pts
	# would have to create new input file with correct geometry ######################
    }

    print "PDF COULD NOT FIND MIN/MAX ANGLE WITH PDF > $PDF_percent \n";
    print "PDF------------------\n\n";

    # if PDF function fails, calculate as absolute max or min
    if ($is_min)
    {
	return (get_minima_scenario($hash_ref));
    }

    return (get_maxima_scenario($hash_ref));
}

sub test_PDF
{
    my %hash = (120.0000 => 0.1489*1000,
		135.0000 => 0.3949*1000,
		150.0000 => 1.2196*1000,
		165.0000 => 1.8692*1000,
		180.0000 => 1.9801*1000,
		195.0000 => 1.7797*1000,
		210.0000 => 1.5421*1000,
		225.0000 => 1.4045*1000,
		240.0000 => 1.4045*1000,
		255.0000 => 1.1909*1000);
 
    my $prev_work = 0;

    my ($ang, $work) = get_PDF_forecast(\%hash, $prev_work, 'max 0.95', 1);
    print "angle, work = $ang $work \n";
...
# called: %this_fault = remove_not_slipping_ineff($is_min, $prev_prop_work, \%this_file, \%this_fault)
...
sub remove_not_slipping_ineff
{
	...
	{
	    print "\nNewly added element not slipping \n\tRemoving geometry of file: $angle_file{$angle} \n"; 
	    push(@removed_angles, $angle); 
	    delete $angle_work{$angle};
	}
	elsif (($is_min && ($delwext_A > 0)) || (!$is_min && ($delwext_A < 0)))
	{
	    print "\nNewly added element inefficient \n\tRemoving geometry of file: $angle_file{$angle} \n"; 
	    push(@removed_angles, $angle); 
	    delete $angle_work{$angle};
	}
    ...
    {
	print "\nAngles that did not slip or inefficient:\n";
...
sub test_coul
{
   my $not_slipped_element = is_Coulomb_slip('specsT10S10_1_seg0_2_180.out');
...
sub get_first_last
{
	...
	elsif ($line =~ /tip BE/ || $line =~ /Checking/)
	{
	    close HANDLE;
...
sub test_inter_cor
{
    my %adding;
    $adding{'rough1 2'} = 45;
    my $inter_struct = get_intersect_structs('debug_165.in');
    my $res = correct_intersections('debug_165.in', \%adding, 0, $inter_struct);
    print "result: $res \n";
}
...
sub correct_intersections
{
	...
	my $n1 = $names[0];
	my $n2 = $names[1];
	print "\nWARNING: Fracture $n1 intersects itself \n";
	#die;
	return 1; 
    }
...
sub test_adjust
{
    my @new_ref = (18000, 1720, 1000, 1000);
    my $old_coors = '17457.03350703	17940.46404455	17225.56951756	17846.09725112';
    adjust_through_going(0, 'test_adjust.in', $old_coors, \@new_ref);
}
...
sub adjust_through_going
{
	...
	elsif ($found && $line =~ /^\d+/)
	{
	    ...
	    {
		#print "not bounds, read some lines \n";
...
sub reset_flaw_one
{
   ...
   while (<FILE_HANDLE>)
	...
	elsif ($fault_props ne "" && $found_fault && $this_Line =~ /^1\s+/) ####################1ELEM
	{
...
sub make_input_file
{
	...
	# if line starts with fracture, but not this fault
	# then signal we have not found the fault 
	elsif (!$is_sand && $this_Line =~ /^fracture\s+/ )
	{      
	    $found_fault = 0;
	    $found_fault_props = 0;
	}
